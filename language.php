...
<?php       //
       ...
       // language 
            $query1 = "SELECT * FROM booklanguage
                       ORDER BY lan_id";
...
<html lang="en">
  ...
  <!-- Preference Language -->
<form method="post" action="language.php?page=<?php echo $page; ?>" class="form-inline">
  <label class="mr-sm-2" for="inlineFormCustomSelectPref">Language Preference</label>
  <select name="alloption" class="custom-select mb-2 mr-sm-2 mb-sm-0" id="inlineFormCustomSelectPref">
  
<?php 
         while($row1 = $result1->fetch_assoc()){  
          $lanid=$row1['lan_id'];
          $lan=$row1['book_language'];  
          ?>
          <option value="<?php echo $lanid ?>"><?php echo $lan ?></option>
         <?php 
        }
